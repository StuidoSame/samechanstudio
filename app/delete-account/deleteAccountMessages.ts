...
export type DeleteAccountMessages = {
  ...
  form: {
    ...
    privacyNotice: string; privacyLink: string; submit: string;
    errors: { app: string; email: string; login: string; consent: string };
    draftReady: string; draftInstruction: string; reference: string; maskedEmail: string;
    directEmail: string; mailAria: string;
  };
  scope: {
  ...
  links: { privacy: string; support: string; home: string };
};
...
const ko: DeleteAccountMessages = {
  ...
  form: {
    ...
    submit: "삭제 요청 메일 작성", errors: { app: "삭제를 요청할 앱을 선택해주세요.", email: "올바른 이메일 주소를 입력해주세요.", login: "실제로 사용한 로그인 방식을 선택해주세요.", consent: "삭제 및 복구 제한 안내에 동의해야 합니다." },
    draftReady: "삭제 요청 메일 초안을 준비했습니다.", draftInstruction: "메일 앱에서 전송을 완료해야 요청이 접수됩니다. 이 화면만으로는 삭제 요청이 전송되거나 완료되지 않습니다.", reference: "참조 번호", maskedEmail: "확인 이메일", directEmail: "메일 앱이 열리지 않나요? contact@samestudio.kr", mailAria: "contact@samestudio.kr로 계정 삭제 요청 이메일 보내기",
  },
  scope: {
  ...
  links: { privacy: "PRIVACY", support: "SUPPORT", home: "HOME" },
};
...
const zhCN: DeleteAccountMessages = {
  ...
  form: { ...en.form, title: "填写删除申请", intro: "目前本网站确认支持账号删除的应用仅为 ODOW。请勿输入密码、验证码或身份证件信息。", app: "应用", email: "注册邮箱", login: "登录方式", loginPlaceholder: "请选择", requestType: "申请类型", fullDeletion: "删除账号及全部关联数据", message: "补充信息（可选）", messagePlaceholder: "仅填写核验账号所需的信息，请勿输入密码或验证码。", consent: "我已了解账号删除完成后相关数据可能无法恢复，并申请删除上述账号。", privacyNotice: "填写的邮箱、应用、登录方式和申请内容仅用于处理删除申请及身份核验。申请记录依照隐私政策和适用法律保留。", privacyLink: "隐私政策", submit: "提交删除请求", errors: { app: "请选择应用。", email: "请输入有效的邮箱地址。", login: "请选择实际使用的登录方式。", consent: "必须同意删除及无法恢复提示。" }, draftReady: "删除申请邮件草稿已准备好。", draftInstruction: "必须在邮件应用中完成发送，申请才会被接收。仅停留在此页面不会发送申请或完成删除。", reference: "参考编号", maskedEmail: "确认邮箱", directEmail: "邮件应用未打开？contact@samestudio.kr", mailAria: "向 contact@samestudio.kr 发送账号删除申请" },
...
const zhTW: DeleteAccountMessages = {
  ...
  form: { ...zhCN.form, title: "填寫刪除申請", intro: "目前本網站確認支援帳號刪除的應用程式僅為 ODOW。請勿輸入密碼、驗證碼或身分證件資訊。", app: "應用程式", email: "註冊電子郵件", login: "登入方式", loginPlaceholder: "請選擇", requestType: "申請類型", fullDeletion: "刪除帳號及全部關聯資料", message: "補充資訊（選填）", messagePlaceholder: "僅填寫核驗帳號所需的資訊，請勿輸入密碼或驗證碼。", consent: "我已瞭解帳號刪除完成後相關資料可能無法復原，並申請刪除上述帳號。", privacyNotice: "填寫的電子郵件、應用程式、登入方式和申請內容僅用於處理刪除申請及身分核驗。申請記錄依隱私權政策和適用法律保留。", privacyLink: "隱私權政策", submit: "提交刪除請求", errors: { app: "請選擇應用程式。", email: "請輸入有效的電子郵件地址。", login: "請選擇實際使用的登入方式。", consent: "必須同意刪除及無法復原提示。" }, draftReady: "刪除申請郵件草稿已準備好。", draftInstruction: "必須在郵件應用程式中完成傳送，申請才會被接收。僅停留在此頁面不會傳送申請或完成刪除。", reference: "參考編號", maskedEmail: "確認電子郵件", directEmail: "郵件應用程式未開啟？contact@samestudio.kr", mailAria: "向 contact@samestudio.kr 傳送帳號刪除申請" },
